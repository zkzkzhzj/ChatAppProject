...
7. **트레이드오프 논의가 발생하면 그 응답 안에서 즉시 학습노트를 작성하라.** 대화 중 "A vs B 중 뭘 쓸까", "왜 이 방식인가", "다른 방법은 없나" 같은 기술 선택·비교가 나오면, 해당 응답을 끝내기 전에 learning-agent를 호출하여 `docs/learning/`에 학습노트를 남긴다. "나중에 쓰지"는 "안 쓴다"와 같다. 번호는 기본적으로 기존 마지막 번호 + 1이지만, **트랙 작업 중일 때는 `docs/learning/RESERVED.md`에 자기 트랙으로 예약된 번호 중 가장 작은 미사용 번호**를 사용한다 (§8 Parallel Tracks · `docs/conventions/parallel-work.md` 참조).
8. **커밋/PR 전에 메모리와 인수인계 문서를 반드시 최신화하라.** 코드를 올리기 전에 `memory/` 파일과 인수인계 문서가 현재 작업 내용을 반영하고 있는지 확인한다. 다음 세션이 이 문서만 보고 이어서 작업할 수 있어야 한다. 갱신 대상은 작업 맥락에 따라 다르다: **단일 작업이면 `docs/handover.md` 직접 갱신**, **트랙 작업 중이면 메인 `docs/handover.md`는 건드리지 않고 자기 트랙의 `docs/handover/track-{id}.md`만 갱신** (메인 갱신은 트랙 머지 시점에만, §8 Parallel Tracks · `docs/conventions/parallel-work.md` 참조).
