...
> 반복된 사용자 피드백 — 2026-05-11. 이 룰은 다른 모든 룰보다 먼저 적용한다.

**금지 단어 (응답 본문에 사용 금지)**:
- `결박` — 0회. 동사 의미(묶다)와도 안 맞고 음운 잡음.
- `결` — 의존명사처럼 쓰지 않는다. 정확한 단어로(`방식`·`방법`·`이유`·`쪽`·`곳`·`결과`·`시점`·`관점`).
- `박다`/`박았다`/`박혀있다`/`박는다`/`박힘`/`박힌` 등 — 한 응답에 2회 초과 사용 금지. 명확한 단어로(`추가하다`·`만들다`·`정하다`·`적용하다`·`커밋하다`·`머지하다`·`있다`·`존재한다`).

**응답 작성 원칙**:
- 한 문장 = 한 줄. 긴 단락 금지.
- 표·코드 블록·번호 list 위주. 산문 단락 최소화.
- 응답 길어질수록 filler 위험 ↑ — 짧게 끊는다.
- 응답 종료 전 마음속 self-check: 위 금지 단어가 박혔는지 점검.

**위반 시 사용자 피드백 즉시 반영** — memory `feedback_korean_output_quality.md` + `feedback_style.md` 정합.
