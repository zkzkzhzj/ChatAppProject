...
>
> 2026-05-26 이후 Codex가 주력 실행 주체다. Claude Code 세팅은 폐기하지 않고
> 기존 훅·스킬·에이전트 자산으로 보존한다. 공통 하네스 SSoT는
> [`docs/harness/README.md`](./docs/harness/README.md) 를 우선 참조한다.
...
- Codex와 Claude가 공유해야 하는 운영 규칙은 `docs/harness/`를 우선한다. Claude 전용 세부 훅과 스킬은 이 문서와 `.claude/`에 남긴다.
