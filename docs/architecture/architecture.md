...
- Confession/Letter: PostgreSQL + Kafka
- Village visit/presence: PostgreSQL + WebSocket + Redis
...
public class ConfessionLetter {
    ...
    public void markRead(Long readerUserId) {
        if (!this.receiverUserId.equals(readerUserId)) {
            throw new LetterAccessDeniedException(this.id, readerUserId);
        }
...
public class SendConfessionLetterService implements SendConfessionLetterUseCase {
    ...
    private final SaveLetterResultPort saveLetterResultPort;
    private final LoadConfessionPort loadConfessionPort;
    ...
    @Override
    @Transactional
    public LetterResult execute(SendConfessionLetterCommand command) {
...
@Repository
@RequiredArgsConstructor
public class ConfessionLetterJpaAdapter implements LoadConfessionLetterPort, SaveConfessionLetterPort {
    private final ConfessionLetterJpaRepository jpaRepository;
    private final ConfessionLetterMapper mapper;

    @Override
    public ConfessionLetter load(Long letterId) {
        ConfessionLetterJpaEntity entity = jpaRepository.findById(letterId)
                .orElseThrow(() -> new ConfessionLetterNotFoundException(letterId));
        return mapper.toDomain(entity);
    }

    @Override
    public void save(ConfessionLetter letter) {
        // JPA @Version으로 낙관적 락 — 동시 갱신 시 OptimisticLockingFailureException 발생
        jpaRepository.save(mapper.toEntity(letter));
    }
}
```
...
public interface LoadConfessionLetterPort {
    ConfessionLetter load(Long letterId);
}
...
public interface ConfessionLetterPersistencePort {
    ConfessionLetter load(Long letterId);
    void save(ConfessionLetter letter);
}
```
...
```java
public class ConfessionLetterJpaAdapter implements ConfessionLetterPersistencePort {
    // load, save 모두 구현
}
```
...
public interface SendConfessionLetterUseCase {
    LetterResult execute(SendConfessionLetterCommand command);
}

public interface LoadVillageVisitSummaryUseCase {
    VillageVisitSummary execute(LoadVillageVisitSummaryQuery query);
}
```
...
- **Command**: 상태 변경. `SendConfessionLetterCommand`, `CreateSuggestionCommand`, `RecordVillageVisitCommand`, `SendMessageCommand`
- **Query**: 조회만. `LoadConfessionDetailQuery`, `LoadVillageVisitSummaryQuery`, `LoadSuggestionListQuery`
...
```java
@Component
public class ConfessionLetterMapper {

    public ConfessionLetter toDomain(ConfessionLetterJpaEntity entity) {
        // restore() — DB에서 읽어온 기존 데이터를 Domain으로 복원한다.
        // @Builder 대신 정적 팩토리 메서드를 사용한다. (coding.md 5.1 참조)
        return ConfessionLetter.restore(
                entity.getId(),
                entity.getConfessionId(),
    ...
    public ConfessionLetterJpaEntity toEntity(ConfessionLetter domain) {
        // Persistence Entity도 정적 팩토리 메서드로 생성한다. (coding.md 5.2 참조)
        return ConfessionLetterJpaEntity.from(domain);
    }
}
```
...
CREATE TABLE outbox_event (
    ...
    event_type      VARCHAR(100) NOT NULL,    -- 'CONFESSION_LETTER_SENT', 'REPORT_THRESHOLD'
