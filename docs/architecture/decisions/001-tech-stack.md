...
> 작성일: 2026-04-03
> 상태: 확정
> supersession: 2026-06-23 issue #151 기준, Phaser/픽셀 기반 마을과 store/economy UI 범위는 현재 제품 방향이 아니다. 현재 프론트 런타임은 Next.js + React + Three.js + Howler.js 기반 3D 마을/도서관이며, 저장형 개인화/Economy는 제거됐다.
...
동시성 제어(MVCC)가 더 성숙. MySQL은 undo log 기반, PostgreSQL은 다중 버전 직접 유지. 편지 상태 변경, 신고 접수, 방문 집계 같은 동시 쓰기 도메인에 적합. JSON 타입 지원 우수. 국내 Node.js·풀스택 백엔드 진영의 메이저 스택과 일치.
...
도메인 간 비동기 이벤트 처리. 편지 도착 알림, 신고→제재 같은 부수 효과를 메인 트랜잭션과 분리. KRaft 모드 지원 버전(Confluent 7.6.0)을 사용하여 Zookeeper 없이 단독 구동 가능.
...
Next.js: 프론트엔드 프레임워크. 라우팅, SSR 등을 프레임워크가 제공하여 백엔드 개발자가 AI 도움을 받아 작업하기에 적합. React는 일반 UI와 도서관/고백 표면을 담당하고, Three.js는 3D 마을/도서관 런타임 렌더링을 담당한다. Howler.js는 환경음 제어에 사용한다.

## Phaser.js 선택 이유 (vs PixiJS) — 과거 결정

초기 2D 시점에는 Phaser를 게임 프레임워크로 선택했다. 물리엔진, 씬 관리, 입력 처리가 내장되어 PixiJS보다 직접 구현 부담이 낮았기 때문이다.

이 결정은 `village-3d` 트랙에서 Three.js로 대체됐다. 현재 구현 지침으로 사용하지 않는다.

## 픽셀(도트) 아트 — 과거 결정

초기 2D 시점에는 픽셀아트를 검토했다. 이후 3D 안식처 방향으로 전환되어 현재 시각 전략은 low-poly 3D, warm lighting, fog, 환경음 중심이다.
