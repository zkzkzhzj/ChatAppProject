...
자동 fix-loop 안에서 사용자가 끼어드는 마지막 지점. `/step-start` 가 위험한 작업 커밋 직전에 발동.

> "코드는 작동하지만 네 것이 아니게 된다" — 자동화의 함정.
> 게이트는 **본인 말로 답하기** 를 강제해 결정 누락을 막는다.

위치: `/step-start` 스킬 안. 작업 범위 확정 → 필요한 선행 검토 → **여기** → 구현/커밋.
...
> 예: #1 동시성(B) + #6 DB 스키마(C) 동시 매칭 → 최종 C. 한 작업 커밋에 여러 결정이 섞이면 가장 깊은 검증을 적용한다.
...
| `/step-start` SKILL.md | 게이트의 호출 지점. 위험한 작업 커밋을 시작하기 전 발동 |
...
| 2026-05-30 | 발동 지점을 PR 생성 직전에서 위험한 작업 커밋 직전으로 교정 | `library-confession-mvp` |
