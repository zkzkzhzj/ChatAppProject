...
| 파일/디렉토리 | 분리 방법 |
|--------------|-----------|
| `docs/handover.md` (메인) | **수정 금지.** 트랙 머지 시점에만 갱신 |
| `docs/handover/track-*.md` | 트랙별 별도 파일이라 자연 분리 |
...
- [ ] 자기 트랙의 `track-{id}.md` 갱신 (Step 완료 표시 등)
- [ ] 메인 `docs/handover.md`의 "활성 트랙 인덱스"에 자기 트랙 상태 반영 (필요 시)
- [ ] RESERVED.md의 사용한 번호 "사용 완료" 표시 (해당하는 경우)
...
2. `docs/handover/INDEX.md`에서 "활성 트랙"에서 제거 (또는 별도 "완료 트랙" 섹션 신설 가능)
3. 메인 `docs/handover.md` 전체 완료 요약 표에 한 줄 추가
