...
| 트랙 ID | 파일 | 상태 | 이슈 |
|---------|------|------|------|
| — | — | (활성 트랙 없음 — `ws-redis` Step 3 착수 시 재추가) | — |

**최근 종료 트랙** (시간 역순):

- `ws-redis` Step 2 (PR #26, 2026-04-27) — raw WS + Redis Pub/Sub 백엔드. 토폴로지 ③ 결정. (관련 학습노트는 PR #26 머지 후 별도 PR로 인덱스 보강 예정)
- `ghost-session` (PR #36 · #37, 2026-04-27) — presence cleanup 진단 + 게스트 토큰 stale fix 5종. (관련 학습노트는 PR #36·#37 머지 후 별도 PR로 인덱스 보강 예정)
...
- `member-token-renewal` — 멤버 토큰 자동 갱신 (sliding session vs refresh vs WS push)
...
| ws-redis Step 2 | raw WS + Redis Pub/Sub 백엔드 (토폴로지 ③ 결정) | PR #26 | [44](./learning/44-spring-stomp-external-broker-choice.md) · [45](./learning/45-websocket-redis-pubsub-redesign.md) · 46·53·59 (PR #26 머지 후 별도 PR로 추가) |
| ghost-session | presence cleanup 진단 + 게스트 토큰 stale fix 5종 | PR #36 · #37 | 54 (PR #36·#37 머지 후 별도 PR로 추가) |
