...
- `harness-spec-driven` (현재 PR, 2026-04-30) — spec-driven 4층 분리 모델 (Issue/Spec/Track/Step) + 자동 fix-loop + Comprehension Gate (13 카테고리/Tier A·B·C) + wiki 활용 강화 4종 + Dependabot 도입 (dependency-tracker-agent 대체). learning 66·67·68
- `infra-tls-hardening` (현재 PR, 2026-04-28) — Cloudflare SSL Flexible→Full(strict), Origin CA, HSTS, 보안그룹 prefix list, nginx 표준 구조 마이그. learning 65
...
| infra-tls-hardening | Cloudflare SSL Flexible→Full(strict), Cloudflare Origin CA + EC2 nginx 인증서, HSTS 6mo, 보안그룹 80/443 → Cloudflare IP prefix list, nginx.conf 비표준 → sites-enabled 표준 구조 마이그 | (현재 PR) | [65](./learning/65-cookie-security-attributes-deep-dive.md) |
| harness-spec-driven | AI Native 하네스 spec-driven 4층 (Issue/Spec/Track/Step) + 자동 fix-loop (테스트 3회·리뷰 2회·PR 게이트 2회) + Comprehension Gate (13 카테고리/Tier A·B·C) + wiki 활용 강화 4종 + Dependabot (dependency-tracker-agent 대체) + 슬래시 스킬 4종 (`/spec-new`·`/track-start`·`/step-start`·`/track-end`) | (현재 PR) | [66](./learning/66-spec-driven-fix-loop-comprehension-gate.md) · [67](./learning/67-wiki-policy-rejection-reversal.md) · [68](./learning/68-npc-service-differentiator-adr.md) |
...
- 현재 활성 트랙 없음.
- 다음 착수 후보는 §1의 `다음 후보 트랙` 목록을 기준으로 사용자 승인 후 시작 (`token-auto-renewal`, 디자인 에셋·UI 꾸미기, `ws-redis` Step 3, `multi-session-policy`, Phase 4 Economy, `s3-media`).
