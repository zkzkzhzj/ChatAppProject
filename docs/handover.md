...
### Phase 6+ — 공간·캐릭터 개인화 (아이디어)

마음의 고향 핵심 가치("내 안식처") 강화용. Week 7 종료 후 순차 검토.

- **집 배경 이미지 생성** — Nano Banana(Gemini 2.5 Flash Image) API 로 유저 프롬프트 → 개인 공간(집) 배경 이미지 생성. 생성 결과를 S3 저장, `Space` Entity 의 `backgroundImageUrl` 필드로 연결. 유해/저작권 프롬프트 필터링·일일 생성 쿼터·fallback 이미지 설계 필요.
- **캐릭터 생성** — 유저가 자기 캐릭터를 프롬프트로 만들거나 프리셋 조합으로 커스터마이즈. Phaser 스프라이트 시트 자동 생성 경로 혹은 2D 아바타 라이브러리 연동. 기본 NPC 캐릭터와 구별을 위한 character_type enum 확장 가능.

두 기능 모두 **이미지 생성 비용 관리**(유저당 일일 쿼터, 생성 실패 시 fallback)가 가장 중요한 설계 포인트.
