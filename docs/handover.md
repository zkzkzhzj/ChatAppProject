...
| 유령 캐릭터 fix | #28 본인 따라다니는 분신 — 게스트 토큰 재발급 시 자기인식 stale + tokenBridge 도입 + 게스트 만료 24h 분리 | PR #36 | [54](./learning/54-presence-cleanup-ghost-character-diagnosis.md) |
...
- **멤버 토큰 자동 갱신** — refresh token / sliding session / WS push 패턴 비교 + 게스트 sessionId 분리. 멤버 만료 시 재로그인 UX. (`ghost-session` 트랙 종료 시 분리 결정, [learning/54](./learning/54-presence-cleanup-ghost-character-diagnosis.md) 참조)
- **다중 세션 정책** — 동일 `userId` 멀티 디바이스/멀티 탭 동시 접속 (대체/거부/병행). `ws-redis` SessionRegistry 와 자연 연계 가능
