...
| Phase 2 | Village 초기 모델 (issue #151로 저장형 개인화 제거 완료) | PR #7 | [13](./learning/13-global-alert-port-pattern.md) |
...
SendConfessionLetterService
  ...
  → OutboxKafkaRelay (@Scheduled 1s) → Kafka "confession.letter.sent" 토픽
  → ConfessionLetterSentEventConsumer → 수신자 메일 알림 큐 전달
```
...
│   └── adapter/
├── village/
│   ├── domain/          ← 런타임 마을/presence 경계 (저장형 개인화 제거 완료)
