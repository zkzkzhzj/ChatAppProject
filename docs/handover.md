...
- STOMP 엔드포인트: `/ws` (SockJS fallback) — **운영 기본**
...
- 현재 활성 트랙 없음. (`realtime-infra-reset` 2026-06-11 종료 — ADR-010)
- 다음 착수 후보는 §1의 `다음 후보 트랙` 목록을 기준으로 사용자 승인 후 시작 (`realtime-raw-ws-cutover`, `token-auto-renewal`, village-3d 후속 Step 트랙, `multi-session-policy`, Phase 4 Economy).
