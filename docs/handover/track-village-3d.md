...
### 다음 진행 — Step 1.5 멀티유저 위치 동기화

- 옛 Phaser `sendPosition` / `onPositionUpdate` / `onTypingUpdate` 결을 Three.js Scene에 통합
- 다른 유저 박스 placeholder 렌더 (Step 3 캐릭터 모델 시점에 교체)
- WS 연결 + 위치 broadcast (50~100ms throttle) + 입장/퇴장 이벤트
- Codex P1 (PR #68) 회수 항목
