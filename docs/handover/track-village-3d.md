...
### Step 1.7 — 채팅 UI 재설계 (🟢 작업 완료, 검증 중)

브랜치: `feat/village-3d-step1.7-chat-bubble-redesign` (base=`feat/village-3d`).

**무엇 (구현됨)**:
- `frontend/src/three/chat/SpeechBubble.ts` — `THREE.Sprite` + `CanvasTexture` (DPR 2배, 둥근 사각형 + 꼬리, 22자 말줄임), 6초 timer + `setText` 재사용 + 명시 dispose
- `frontend/src/three/chat/ChatInputAnchor.tsx` — Enter/Escape 토글, Vector3→screen coords 매 프레임 변환, 화면 가장자리 clamp, `ChatInput` 재사용 (`onSent` prop)
- `frontend/src/components/chat/ChatDrawer.tsx` — 우측 사이드 슬라이드 인 (transition `translateX`), 토글 버튼 (우측 하단), 옛 `ChatMessageList`·`ChatBubble`·`LoginPrompt` 재사용
- `frontend/src/lib/websocket/chatBridge.ts` — `emitChatMessage` / `onChatMessage` 다중 리스너 (positionBridge·tokenBridge와 동일 패턴)
- `Character.attachBubble` / `RemotePlayer.attachBubble` — 기존 bubble 재사용 (setText) 또는 신규, 만료 시 자동 group.remove + dispose
- `SceneManager.applyChatMessage(msg)` — `senderType=USER` 메시지만 머리 위 표시. `senderId → user-{id}` 매핑 (백엔드 `PositionUpdateEvent.displayId` 정합)
- `useStomp` — 메시지 수신 시 `chatBridge.emit` 같이 호출
- `ThreeGame.onReady` — `GameLoader` 결로 SceneManager hoist (ChatInputAnchor 에 props 전달)
- `page.tsx` — `<ChatOverlay />` 제거. `GameLoader`가 `<ChatInputAnchor />` + `<ChatDrawer />` 직접 mount
- `ChatOverlay.tsx`·`ChatOverlay.test.tsx` 폐기

**테스트**:
- 단위 테스트 70/70 통과 (SpeechBubble 6 + chatBridge 3 + 기존 61)
- `npm run build` 통과
- typecheck / lint 통과
- backend 변경 X — rebuild 불필요

**spec.decisions**: D12(3D Sprite) / D13(인라인 입력) / D14(우측 드로우어) / D15(6초) 모두 코드 결로 실현. learning 74 결로 트레이드오프 학습노트 별도 박힘.

**의도적 한계**:
- 게스트 메시지 표시 X — `senderId` 없음 (백엔드 contract 결로 게스트는 채팅 발송 자체 차단됨)
- NPC·SYSTEM 메시지는 머리 위 표시 X — ChatDrawer 내역에만 (혼란 방지)
- 모바일 대응 X (D11 데스크탑 우선)
