...
| `superpowers-adoption.md` | Superpowers 방법론 중 하네스에 흡수할 트리거와 보류할 것 |
| `log.md` | 하네스 규칙 변경 이력 |

문서 frontmatter는 [document-frontmatter.md](../conventions/document-frontmatter.md)의
OKF-lite 규칙을 따른다. 전체 OKF 호환이 아니라 `type`, `description`, `tags` 중심의
최소 메타데이터 계약이다.
...
- Superpowers: brainstorming, writing-plans, worktree isolation, parallel agents,
  code review, branch finishing을 자동 트리거하는 스킬 기반 개발 방법론
- Google Cloud knowledge-catalog OKF SPEC: Markdown frontmatter, index, log 중심의
  에이전트 친화 문서 메타데이터 패턴
