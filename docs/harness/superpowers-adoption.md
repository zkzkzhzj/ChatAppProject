...
| 프로토타입, UX, UI 디자인, 편의성 개선, 기능 개선이 한 요청에 묶임 | 구현하지 말고 목표, 우선순위, 디자인 방향, 첫 PR 범위를 먼저 정렬 |
...
6. 넓은 제품·디자인 요청은 "작업 요청"처럼 보여도 기본 구현 경로로 넘기지 않는다.
   먼저 alignment brief를 작성하고 사용자 확인을 받은 뒤 계획/구현으로 전환한다.
7. superpowers 흐름의 산출물(`docs/superpowers/specs/`, `docs/superpowers/plans/`)은
   작업 설계 문서이고, 기능 spec의 정본 위치는 `docs/specs/features/{feature}.md`
   (spec-driven 4층 모델)로 유지한다. 트랙 파일의 `Spec:` 링크는 정본이 있으면 정본을,
   설계 문서만 있으면 그 경로를 가리킨다. Claude session-start 훅은 `docs/` 하위
   임의 경로를 인식하므로 어느 쪽이든 탐지된다.
