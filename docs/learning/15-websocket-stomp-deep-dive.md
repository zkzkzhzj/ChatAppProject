...
```
/app/chat/{roomId}         ← 클라이언트 → 서버 (메시지 전송)
/topic/chat/{roomId}       ← 서버 → 클라이언트 (NPC 응답 broadcast)
```
...
클라이언트가 `/app/chat/1`로 메시지를 보낼 때 Spring 내부에서 일어나는 일:

```
클라이언트 SEND /app/chat/1
    │
    ▼
ChannelInterceptor (인증 검사)
    │
    ▼
@MessageMapping("/chat/{roomId}")   ← /app prefix 제거 후 매핑
...
SendMessageUseCase.execute()
    ...
    │
    ▼
messagingTemplate.convertAndSend("/topic/chat/1", npcMessage)
    │
    ▼
Simple Broker → /topic/chat/1 구독 중인 모든 클라이언트에게 MESSAGE 프레임 전송
```
...
```
경로 A: REST POST /api/v1/chat-rooms/{id}/messages
경로 B: STOMP   /app/chat/{roomId}
```
...
| 항목 | REST | STOMP |
|------|------|-------|
| 응답 방식 | HTTP 응답으로 userMessage + npcMessage 동시 반환 | 응답 없음, 구독 채널로만 수신 |
| NPC 응답 broadcast | 있음 (SimpMessagingTemplate 직접 호출) | 있음 (동일) |
...
SUBSCRIBE /topic/chat/1
    → Simple Broker: "세션 abc123이 /topic/chat/1 구독 중" 을 Map에 기록

MESSAGE /topic/chat/1
    → Simple Broker: Map에서 /topic/chat/1 구독자 목록 조회
...
public void handleMessage(
        ...
        @AuthenticationPrincipal AuthenticatedUser user) {  // JWT에서 파싱된 유저
...
client.connect({ Authorization: 'Bearer ' + token }, () => {
  ...
  client.subscribe('/topic/chat/' + chatRoomId, (msg) => {
    console.log('NPC 응답:', JSON.parse(msg.body));
  });

  // 메시지 전송 (STOMP 경로)
  client.send('/app/chat/' + chatRoomId, {}, JSON.stringify({ body: '안녕하세요' }));
});
```
...
```
/topic/chat/{roomId}   → 채팅방 참여자 전체 broadcast (1:N)
