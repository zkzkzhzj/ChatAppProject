...
- **[#08. Phase 1 계층 설계 패턴](./08-phase1-layer-patterns.md)** — 본 노트가 다시 적용한 정적 팩토리·Port 원칙의 출발점. 본 노트는 "원칙은 알고 있었는데 운영하다 발견한 함정"을 다룸
- **[#53. outbound port 호출자 룰](./53-hexagonal-outbound-port-caller-rule.md)** — 본 노트의 "책임 경계" 개념을 호출자 기준의 port 정의로 확장 (ws-redis Step 2)
