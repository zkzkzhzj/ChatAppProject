...
aws ssm send-command \
  --parameters "{\"commands\":[\"cd /home/ubuntu/... && docker compose pull && ...\"]}"
```

**파일 호출** (채택):
```bash
aws ssm send-command \
  --parameters "{\"commands\":[\"sudo -u ubuntu bash /home/ubuntu/ChatAppProject/scripts/deploy.sh $APP_TAG $FRONTEND_TAG\"]}"
```
...
- **자기 업데이트**: `git reset --hard origin/main` 이 스크립트 안에서 수행되므로 매 배포마다 스크립트도 최신화
