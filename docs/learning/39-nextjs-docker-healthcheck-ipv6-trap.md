...
정확히 연결이 되지 않는 이유다. 브라우저는 다르다. 브라우저 → Cloudflare → EC2(호스트 nginx) → 컨테이너의 포트매핑 경로로 들어오면 호스트의 Docker가 매핑한 포트(3000)를 통해 컨테이너 내부에 닿는데, Next.js가 `::1`에만 바인딩돼 있으면 그것도 안 돼야 한다. 그런데 왜 브라우저는 됐을까?
...
- 외부 노출은 `docker-compose.yml`의 `ports:` 매핑이 결정한다. 우리는 EC2 앞에 호스트 nginx + Cloudflare를 두고 트래픽을 받으므로 `HOSTNAME=0.0.0.0`은 컨테이너 내부 바인딩일 뿐, 외부 공격면을 늘리지 않는다. (추가로 호스트 측 `ports`를 `127.0.0.1:3000:3000`으로 제한하면 EC2 공인 IP 직접 접근까지 차단할 수 있다 — 후속 개선 과제.)
