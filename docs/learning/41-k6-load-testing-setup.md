...
업계 일부 대형 서비스는 사내에 **nGrinder**가 정착되어 있다. 본 프로젝트가 k6를 고른 이유:

- 개인 프로젝트는 컨트롤러/에이전트 분리의 이득이 없다. 단일 EC2에 단일 바이너리가 충분
- CI(GitHub Actions)에서 스크립트 한 줄로 돌릴 수 있는 게 k6
- 도구 선택은 *환경*에 따라 달라진다 — 팀 환경이 다르면 결정도 다름

### Node.js 진영에서 k6의 자연스러움 (참고)

JS 기반 테스트 스크립트는 *팀의 공용어*에 부합. 백엔드 엔지니어가 팀의 공용어로 테스트 자산을 남길 수 있다는 점은 도구 선택의 한 축.
