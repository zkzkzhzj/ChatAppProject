...
- B. Docker volume mount (EC2 호스트로 mp3 보존)
- C. S3 / R2 / CDN로 외부화
...
> Gemini가 "S3 노출 = 무단 데이터 접근 위험"이라고 일반론을 답했는데, **우리 컨텍스트(public asset)와 분리**해야 했다. LLM이 컨텍스트 모르고 답할 때 끌려가지 말 것 (마플 커피챗 인사이트 [marpple_coffee_chat_insights.md]).
...
1. **Edge 캐싱** — 자산 1개 100만 요청로 S3 fetch 1번. origin 부하 99.99% ↓
2. **글로벌 latency** — 600+ edge로 사용자 가까운 곳에서 응답
...
**완전 차단 ≠ CloudFront**. Layer 7은 WAF rate-limit ($5~10/월) 또는 Shield Advanced ($3,000/월) 별건. 우리 단계로 과함.
...
이게 AWS에서 confused deputy 공격 방어 패턴. 신뢰 서비스(CloudFront)가 공격자 거로 위장 못 하게 막는 가드.
...
1. **공개 자산 URL 노출 ≠ 보안 위협** — 정책으로 누구나 GET 의도된로 박힘
...
- 자산 합산 1GB 넘어가면 — CloudFront 캐시 미스 비용 + invalidation 비용 모니터링

---

> 본 결정의 외부 정리는 zlog 블로그 [study/2026-05-19](https://github.com/zkzkzhzj/zlog/blob/main/src/content/blog/study/2026-05-19/index.mdx)로 더 깊이 풀었다 (마스킹로 — 버킷명·distribution ID 등).
