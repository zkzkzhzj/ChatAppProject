...
ADR-010 원표는 9조건 — 아래 표는 채팅·위치·타이핑 parity 3건을 첫 행으로 합쳐 8행으로 압축했다.
...
"테스트 통과"는 ADR-010 의 9개 조건 중 기능 parity 3개를 채울 뿐이다. 단위·통합
테스트는 코드의 약속을 검증하지만, reverse proxy 의 upgrade 헤더 처리나 CD 빌드
arg 의 env 주입은 테스트 밖 세계다.
