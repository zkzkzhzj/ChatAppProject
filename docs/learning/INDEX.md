...
| [87](./87-stomp-retention-raw-ws-cutover-conditions.md) | STOMP 유지 + raw WS 컷오버 조건 (ADR-010) | "제거할 것인가" → "언제 제거 가능한가" — 기능 parity vs 운영 parity 8조건 + env 옵트인 strangler fig + 듀얼 영구화 방지 장치 (트랙 `realtime-infra-reset`) |
