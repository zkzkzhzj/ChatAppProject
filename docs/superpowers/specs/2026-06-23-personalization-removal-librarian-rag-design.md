...
Identity는 현재 트랙에서 `user.registered` 발행을 유지하지 않는다. Village도 더 이상 이 이벤트를 구독해 개인 레코드를 만들지 않는다.
...
해당 테이블의 기존 데이터는 의도적으로 삭제한다. 이 마이그레이션은 비가역적이므로 운영 적용 전 DB 백업과 대상 테이블 데이터 존재 여부 확인이 필요하다.
