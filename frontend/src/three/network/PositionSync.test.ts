...
describe('PositionSync', () => {
  ...
  describe('sendIfChanged', () => {
    it('첫 호출은 송신', () => {
      expect(sync.sendIfChanged(1, 2, 0)).toBe(true);
      expect(sender).toHaveBeenCalledWith(1, 2);
    });

    it('THROTTLE_MS 미만 재호출 안 보냄', () => {
      sync.sendIfChanged(1, 2, 0);
      expect(sync.sendIfChanged(2, 3, THROTTLE_MS - 1)).toBe(false);
      expect(sender).toHaveBeenCalledTimes(1);
    });

    it('THROTTLE_MS 이상 + 위치 변화 통과 시 송신', () => {
      sync.sendIfChanged(1, 2, 0);
      expect(sync.sendIfChanged(2, 3, THROTTLE_MS)).toBe(true);
      expect(sender).toHaveBeenLastCalledWith(2, 3);
    });

    it('THROTTLE_MS 통과해도 위치 변화 임계값 미만이면 안 보냄', () => {
      sync.sendIfChanged(1, 2, 0);
      const tinyDelta = POSITION_EPSILON / 2;
      expect(sync.sendIfChanged(1 + tinyDelta, 2 + tinyDelta, 200)).toBe(false);
      expect(sender).toHaveBeenCalledTimes(1);
    });

    it('Three.js z 를 그대로 백엔드 y 인자로 매핑', () => {
      sync.sendIfChanged(5, -7, 0);
      expect(sender).toHaveBeenCalledWith(5, -7);
    });
  });
  ...
  describe('heartbeat (idle visibility 보강)', () => {
    it('변화 없음 + HEARTBEAT_MS 미만 = 송신 X', () => {
      sync.sendIfChanged(1, 2, 0);
      expect(sync.sendIfChanged(1, 2, HEARTBEAT_MS - 1)).toBe(false);
      expect(sender).toHaveBeenCalledTimes(1);
    });

    it('변화 없음 + HEARTBEAT_MS 도래 = 강제 송신 (idle user broadcast)', () => {
      sync.sendIfChanged(1, 2, 0);
      expect(sync.sendIfChanged(1, 2, HEARTBEAT_MS)).toBe(true);
      expect(sender).toHaveBeenCalledTimes(2);
      expect(sender).toHaveBeenLastCalledWith(1, 2);
    });

    it('HEARTBEAT_MS 후 또 HEARTBEAT_MS 후 = 또 송신 (주기적 박힘)', () => {
      sync.sendIfChanged(1, 2, 0);
      sync.sendIfChanged(1, 2, HEARTBEAT_MS);
      expect(sync.sendIfChanged(1, 2, HEARTBEAT_MS * 2)).toBe(true);
